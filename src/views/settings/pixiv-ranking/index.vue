  ...
  <a-spin class="spin" :loading="loading" tip="加载中..." :size="35">
    <div class="container">
      <a-form ref="formRef" layout="horizontal" size="large" :auto-label-width="true" :scroll-to-first-error="true" :model="formModel">
          ...
          <a-tabs default-active-key="1" @change="onTabChange">
            <a-tab-pane key="1" title="全局">
              ...
              <a-form-item field="maxScan" label="扫描数量" tooltip="每个榜单作品的最大扫描数量" :disabled="!formModel.enable" feedback>
                <a-input-number v-model:model-value="formModel.maxScan" :style="{ maxWidth: '300px' }" :min="1" :max="1000" placeholder="输入一个数字" mode="button" size="large">
                  <template #suffix>个</template>
                </a-input-number>
              </a-form-item>

              <a-form-item field="previewInPage" label="单页显示" tooltip="合成一览图中，每张一览图最多显示多少张图片" :disabled="!formModel.enable" feedback>
                <a-input-number v-model:model-value="formModel.previewInPage" :style="{ maxWidth: '300px' }" :min="1" :max="50" placeholder="输入一个数字" mode="button" size="large">
            ...
            <a-tab-pane key="2" title="日榜">
              ...
              <a-form-item field="daily.commands" label="全年龄指令" tooltip="获取全年龄榜单指令" extra="输入一个指令后按下Enter添加" :disabled="!formModel?.enable || !formModel.daily?.enable" feedback>
                <a-input-tag v-model:model-value="formModel.daily!.commands" :style="{ minHeight: '100px' }" placeholder="输入指令后按下回车添加" allow-clear />
              </a-form-item>

              <a-form-item field="daily.r18Commands" label="R18指令" tooltip="获取R18榜单指令" extra="输入一个指令后按下Enter添加" :disabled="!formModel?.enable || !formModel.daily?.enable" feedback>
              ...
              <a-form-item field="daily.minBookRate" label="最低收藏率" tooltip="榜单作品中的最低收藏率，用于对作品进行过滤，未达到标准的作品将不显示在预览图中" :disabled="!formModel?.enable || !formModel.daily?.enable" feedback>
                <a-input-number v-model:model-value="formModel.daily!.minBookRate" :style="{ maxWidth: '300px' }" :min="0" :max="1" :precision="2" :step="0.01" placeholder="输入一个小数" mode="button"
                  size="large">
                </a-input-number>
              </a-form-item>
              ...
              <a-form-item field="daily.minRatingRate" label="最低点赞率" tooltip="榜单作品中的最低点赞率，用于对作品进行过滤，未达到标准的作品将不显示在预览图中" :disabled="!formModel?.enable || !formModel.daily?.enable" feedback>
                <a-input-number v-model:model-value="formModel.daily!.minRatingRate" :style="{ maxWidth: '300px' }" :min="0" :max="1" :precision="2" :step="0.01" placeholder="输入一个小数" mode="button"
                  size="large">
                </a-input-number>
              </a-form-item>
            ...
            <a-tab-pane key="3" title="AI日榜">
              ...
              <a-form-item field="dailyAI.commands" label="全年龄指令" tooltip="获取全年龄榜单指令" extra="输入一个指令后按下Enter添加" :disabled="!formModel?.enable || !formModel.dailyAI?.enable" feedback>
                <a-input-tag v-model:model-value="formModel.dailyAI!.commands" :style="{ minHeight: '100px' }" placeholder="输入指令后按下回车添加" allow-clear />
              </a-form-item>

              <a-form-item field="dailyAI.r18Commands" label="R18指令" tooltip="获取R18榜单指令" extra="输入一个指令后按下Enter添加" :disabled="!formModel?.enable || !formModel.dailyAI?.enable" feedback>
              ...
              <a-form-item field="dailyAI.minBookRate" label="最低收藏率" tooltip="榜单作品中的最低收藏率，用于对作品进行过滤，未达到标准的作品将不显示在预览图中" :disabled="!formModel?.enable || !formModel.dailyAI?.enable" feedback>
                <a-input-number v-model:model-value="formModel.dailyAI!.minBookRate" :style="{ maxWidth: '300px' }" :min="0" :max="1" :precision="2" :step="0.01" placeholder="输入一个小数" mode="button"
                  size="large">
                </a-input-number>
              </a-form-item>
              ...
              <a-form-item field="dailyAI.minRatingRate" label="最低点赞率" tooltip="榜单作品中的最低点赞率，用于对作品进行过滤，未达到标准的作品将不显示在预览图中" :disabled="!formModel?.enable || !formModel.dailyAI?.enable" feedback>
                <a-input-number v-model:model-value="formModel.dailyAI!.minRatingRate" :style="{ maxWidth: '300px' }" :min="0" :max="1" :precision="2" :step="0.01" placeholder="输入一个小数" mode="button"
                  size="large">
                </a-input-number>
              </a-form-item>
            ...
            <a-tab-pane key="4" title="受男性欢迎榜">
              ...
              <a-form-item field="male.commands" label="全年龄指令" tooltip="获取全年龄榜单指令" extra="输入一个指令后按下Enter添加" :disabled="!formModel?.enable || !formModel.male?.enable" feedback>
                <a-input-tag v-model:model-value="formModel.male!.commands" :style="{ minHeight: '100px' }" placeholder="输入指令后按下回车添加" allow-clear />
              </a-form-item>

              <a-form-item field="male.r18Commands" label="R18指令" tooltip="获取R18榜单指令" extra="输入一个指令后按下Enter添加" :disabled="!formModel?.enable || !formModel.male?.enable" feedback>
              ...
              <a-form-item field="male.minBookRate" label="最低收藏率" tooltip="榜单作品中的最低收藏率，用于对作品进行过滤，未达到标准的作品将不显示在预览图中" :disabled="!formModel?.enable || !formModel.male?.enable" feedback>
                <a-input-number v-model:model-value="formModel.male!.minBookRate" :style="{ maxWidth: '300px' }" :min="0" :max="1" :precision="2" :step="0.01" placeholder="输入一个小数" mode="button"
                  size="large">
                </a-input-number>
              </a-form-item>
              ...
              <a-form-item field="male.minRatingRate" label="最低点赞率" tooltip="榜单作品中的最低点赞率，用于对作品进行过滤，未达到标准的作品将不显示在预览图中" :disabled="!formModel?.enable || !formModel.male?.enable" feedback>
                <a-input-number v-model:model-value="formModel.male!.minRatingRate" :style="{ maxWidth: '300px' }" :min="0" :max="1" :precision="2" :step="0.01" placeholder="输入一个小数" mode="button"
                  size="large">
                </a-input-number>
              </a-form-item>
            ...
            <a-tab-pane key="5" title="周榜">
              ...
              <a-form-item field="weekly.commands" label="全年龄指令" tooltip="获取全年龄榜单指令" extra="输入一个指令后按下Enter添加" :disabled="!formModel?.enable || !formModel.weekly?.enable" feedback>
                <a-input-tag v-model:model-value="formModel.weekly!.commands" :style="{ minHeight: '100px' }" placeholder="输入指令后按下回车添加" allow-clear />
              </a-form-item>

              <a-form-item field="weekly.r18Commands" label="R18指令" tooltip="获取R18榜单指令" extra="输入一个指令后按下Enter添加" :disabled="!formModel?.enable || !formModel.weekly?.enable" feedback>
              ...
              <a-form-item field="weekly.minBookRate" label="最低收藏率" tooltip="榜单作品中的最低收藏率，用于对作品进行过滤，未达到标准的作品将不显示在预览图中" :disabled="!formModel?.enable || !formModel.weekly?.enable" feedback>
                <a-input-number v-model:model-value="formModel.weekly!.minBookRate" :style="{ maxWidth: '300px' }" :min="0" :max="1" :precision="2" :step="0.01" placeholder="输入一个小数" mode="button"
                  size="large">
                </a-input-number>
              </a-form-item>
              ...
              <a-form-item field="weekly.minRatingRate" label="最低点赞率" tooltip="榜单作品中的最低点赞率，用于对作品进行过滤，未达到标准的作品将不显示在预览图中" :disabled="!formModel?.enable || !formModel.weekly?.enable" feedback>
                <a-input-number v-model:model-value="formModel.weekly!.minRatingRate" :style="{ maxWidth: '300px' }" :min="0" :max="1" :precision="2" :step="0.01" placeholder="输入一个小数" mode="button"
                  size="large">
                </a-input-number>
              </a-form-item>
            ...
            <a-tab-pane key="6" title="月榜">
              ...
              <a-form-item field="monthly.commands" label="全年龄指令" tooltip="获取全年龄榜单指令" extra="输入一个指令后按下Enter添加" :disabled="!formModel?.enable || !formModel.monthly?.enable" feedback>
              ...
              <a-form-item field="monthly.minBookRate" label="最低收藏率" tooltip="榜单作品中的最低收藏率，用于对作品进行过滤，未达到标准的作品将不显示在预览图中" :disabled="!formModel?.enable || !formModel.monthly?.enable" feedback>
                <a-input-number v-model:model-value="formModel.monthly!.minBookRate" :style="{ maxWidth: '300px' }" :min="0" :max="1" :precision="2" :step="0.01" placeholder="输入一个小数" mode="button"
                  size="large">
                </a-input-number>
              </a-form-item>
              ...
              <a-form-item field="monthly.minRatingRate" label="最低点赞率" tooltip="榜单作品中的最低点赞率，用于对作品进行过滤，未达到标准的作品将不显示在预览图中" :disabled="!formModel?.enable || !formModel.monthly?.enable" feedback>
                <a-input-number v-model:model-value="formModel.monthly!.minRatingRate" :style="{ maxWidth: '300px' }" :min="0" :max="1" :precision="2" :step="0.01" placeholder="输入一个小数" mode="button"
                  size="large">
                </a-input-number>
              </a-form-item>
        ...
        <a-card class="card" v-for="(item, index) of formModel.subscribes" :key="index" :title="`推送任务${padLeft((index + 1).toString(), (0).toString(), 2)}`" v-show="showSubscribe">
          <template #extra>
            <a-popconfirm @ok="onDeleteCard(index)" content="确定要删除这个推送任务吗？" type="warning" position="br">
              <span class="delCard">删除</span>
            </a-popconfirm>
          </template>
          <a-form-item :field="`subscribes[${index}].enable`" label="启用推送" tooltip="是否启用该推送功能" :disabled="!formModel?.enable" feedback>
        ...
        <a-card class="card addCard" size="small" :body-style="{ padding: '0px', height: '100%' }" :style="{ marginBottom: '20px' }" v-show="showSubscribe" @click="onCreateCard">
