    ...
    <a-form ref="formRef" layout="horizontal" size="large" :auto-label-width="true" :scroll-to-first-error="true"
      ...
      <a-card class="card" title="功能配置">
        ...
        <a-form-item field="maxWords" label="词汇数量" tooltip="出现在词云中的最大词汇数量" :disabled="!formModel!.enable" feedback>
          <a-input-number v-model:model-value="formModel.maxWords" :style="{ maxWidth: '300px' }" :min="0"
            placeholder="输入一个数字" mode="button" size="large">
        ...
        <a-form-item field="defaultWidth" label="默认宽度" tooltip="矩形词云默认的宽度" :disabled="!formModel!.enable" feedback>
          <a-input-number v-model:model-value="formModel.defaultWidth" :style="{ maxWidth: '300px' }" :min="0"
            placeholder="输入一个数字" mode="button" size="large">
        ...
        <a-form-item field="defaultHeight" label="默认宽度" tooltip="矩形词云默认的高度" :disabled="!formModel!.enable" feedback>
          <a-input-number v-model:model-value="formModel.defaultHeight" :style="{ maxWidth: '300px' }" :min="0"
            placeholder="输入一个数字" mode="button" size="large">
            <template #suffix>像素</template>
          </a-input-number>
        </a-form-item>
        ...
        <a-form-item field="fontPath" label="字体路径" tooltip="默认字体路径" feedback>
          <a-auto-complete v-model:model-value="formModel.fontPath" :data="fontOptions" placeholder="输入一个相对路径或者绝对路径"
            allow-clear />
        </a-form-item>

        <a-form-item field="basicCommands" label="通用指令" tooltip="自定义范围词云命令，未指定范围是默认获取24小时内的词云" extra="输入一个指令后按下Enter添加"
          :disabled="!formModel.enable" feedback>
          <a-input-tag v-model:model-value="formModel.basicCommands" :style="{ minHeight: '100px' }"
            placeholder="输入指令后按下回车添加" allow-clear />
        </a-form-item>

        <a-form-item field="dailyCommands" label="本日指令" tooltip="本日词云命令" extra="输入一个指令后按下Enter添加"
          :disabled="!formModel.enable" feedback>
          <a-input-tag v-model:model-value="formModel.dailyCommands" :style="{ minHeight: '100px' }"
            placeholder="输入指令后按下回车添加" allow-clear />
        </a-form-item>

        <a-form-item field="weeklyCommands" label="本周指令" tooltip="本周词云命令" extra="输入一个指令后按下Enter添加"
          :disabled="!formModel.enable" feedback>
          <a-input-tag v-model:model-value="formModel.weeklyCommands" :style="{ minHeight: '100px' }"
            placeholder="输入指令后按下回车添加" allow-clear />
        </a-form-item>

        <a-form-item field="monthlyCommands" label="本月指令" tooltip="本月词云命令" extra="输入一个指令后按下Enter添加"
          :disabled="!formModel.enable" feedback>
          <a-input-tag v-model:model-value="formModel.monthlyCommands" :style="{ minHeight: '100px' }"
            placeholder="输入指令后按下回车添加" allow-clear />
        </a-form-item>

        <a-form-item field="yearlyCommands" label="本年指令" tooltip="本年词云命令" extra="输入一个指令后按下Enter添加"
          :disabled="!formModel.enable" feedback>
          <a-input-tag v-model:model-value="formModel.yearlyCommands" :style="{ minHeight: '100px' }"
            placeholder="输入指令后按下回车添加" allow-clear />
        </a-form-item>

        <a-form-item field="yesterdayCommands" label="昨日指令" tooltip="昨日词云命令" extra="输入一个指令后按下Enter添加"
          :disabled="!formModel.enable" feedback>
          <a-input-tag v-model:model-value="formModel.yesterdayCommands" :style="{ minHeight: '100px' }"
            placeholder="输入指令后按下回车添加" allow-clear />
        </a-form-item>

        <a-form-item field="lastWeekCommands" label="上周指令" tooltip="上周词云命令" extra="输入一个指令后按下Enter添加"
          :disabled="!formModel.enable" feedback>
          <a-input-tag v-model:model-value="formModel.lastWeekCommands" :style="{ minHeight: '100px' }"
            placeholder="输入指令后按下回车添加" allow-clear />
        </a-form-item>

        <a-form-item field="lastMonthCommands" label="上月指令" tooltip="上月词云命令" extra="输入一个指令后按下Enter添加"
          :disabled="!formModel.enable" feedback>
          <a-input-tag v-model:model-value="formModel.lastMonthCommands" :style="{ minHeight: '100px' }"
            placeholder="输入指令后按下回车添加" allow-clear />
        </a-form-item>

        <a-form-item field="addWordCommands" label="新词指令" tooltip="添加新词命令" extra="输入一个指令后按下Enter添加"
          :disabled="!formModel.enable" feedback>
          <a-input-tag v-model:model-value="formModel.addWordCommands" :style="{ minHeight: '100px' }"
            placeholder="输入指令后按下回车添加" allow-clear />
        </a-form-item>

        <a-form-item field="hideWordCommands" label="删词指令" tooltip="隐藏词汇命令" extra="输入一个指令后按下Enter添加"
          :disabled="!formModel.enable" feedback>
          <a-input-tag v-model:model-value="formModel.hideWordCommands" :style="{ minHeight: '100px' }"
            placeholder="输入指令后按下回车添加" allow-clear />
        </a-form-item>
      ...
      <a-card class="card" v-for="(item, index) of formModel.masks" :key="index"
        ...
        <a-form-item :field="`masks[${index}].width`" label="图片宽度" tooltip="根据模板生成词语的宽度，高度将根据宽度等比例缩放"
          :disabled="!formModel!.enable" feedback>
          <a-input-number v-model:model-value="item.width" :style="{ maxWidth: '300px' }" :min="500" placeholder="输入一个数字"
            mode="button" size="large">
            <template #suffix>像素</template>
          </a-input-number>
        </a-form-item>

        <a-form-item :field="`masks[${index}].name`" label="蒙版名称" tooltip="蒙版名称" :disabled="!formModel.enable"
          :rules="[{ required: true, message: '必须输入一个名称' }]" feedback>
          <a-input v-model:model-value="item.name" placeholder="输入一个名称" allow-clear />
        </a-form-item>

        <a-form-item :field="`masks[${index}].path`" label="蒙版图片" tooltip="蒙版图片路径" :disabled="!formModel.enable"
