  ...
  <div class="container">
    <a-form ref="formRef" layout="horizontal" size="large" :auto-label-width="true" :scroll-to-first-error="true"
      ...
      <a-card class="card" title="本地图源配置">
        ...
        <a-form-item field="localPath" label="图片目录" tooltip="本地涩图的存放目录" :disabled="!formModel?.enable"
          :style="{ marginBottom: '0px' }" feedback>
      ...
      <a-card class="card" title="Lolisuki图源配置">
        <a-form-item label="级别范围" tooltip="Lolisuki Api中的Level，Source为2时可用，值越大表示越涩，最小为0，最大为6，0~4为非R18，5~6为R18"
          :style="{ marginBottom: '0px' }" :disabled="!formModel?.enable" feedback>
      ...
      <a-card class="card" v-for="(item, index) of formModel.timers" :key="index"
        ...
        <a-form-item :field="`timers[${index}].cron`" label="Corn" tooltip="定时器Cron表达式，详细可以百度Cron在线生成"
        ...
        <a-form-item :field="`timers[${index}].source`" label="图片来源" tooltip="图片来源"
          :disabled="!formModel.enable || !item.enable" :rules="[{ required: true, message: '必须选择一个来源' }]" feedback>
          <a-select v-model:model-value="item.source" :options="sourceOptions" placeholder="选择一个图源"
            :scrollbar="true"></a-select>
        </a-form-item>

        <a-form-item :field="`timers[${index}].groups`" label="指定群" tooltip="指定需要推送的群"
