        ...
        <a-card class="card">
          ...
          <a-tabs default-active-key="1">
            ...
            <a-tab-pane key="2" title="Pixiv">
              ...
              <a-form-item field="pixiv.commands" label="指令" tooltip="pixiv涩图指令" extra="输入一个指令后按下Enter添加" :disabled="!formModel.pixiv?.enable" feedback>
                <a-input-tag v-model:model-value="formModel.pixiv!.commands" :style="{ minHeight: '100px' }" placeholder="输入指令后按下回车添加" allow-clear />
              </a-form-item>
              <a-form-item field="pixiv.randomMode" label="随机模式" tooltip="没有指定标签时获取图片的模式" :disabled="!formModel.pixiv?.enable" feedback>
                <a-select v-model:model-value="formModel.pixiv!.randomMode" :options="randomOptions" placeholder="选择一个模式" :scrollbar="true"></a-select>
              </a-form-item>
              <a-form-item field="pixiv.randomTags" label="随机标签" tooltip="随机模式指定为随机标签作品时搜索的标签" :disabled="!formModel.pixiv?.enable" feedback>
              ...
              <a-form-item field="pixiv.minBookRate" label="最低收藏率" tooltip="最低收藏率(收藏数/浏览数)，用于判断涩图是否合格(标签搜索时适用)" :disabled="!formModel.pixiv?.enable" feedback>
                <a-input-number v-model:model-value="formModel.pixiv!.minBookRate" :style="{ maxWidth: '300px' }" :min="0" :max="1" :precision="2" :step="0.01" placeholder="输入一个小数" mode="button"
                  size="large">
                </a-input-number>
              </a-form-item>
            </a-tab-pane>
            <a-tab-pane key="3" title="Lolicon">
              ...
              <a-form-item field="lolicon.commands" label="指令" tooltip="lolicon涩图指令" extra="输入一个指令后按下Enter添加" :disabled="!formModel.lolicon?.enable" feedback>
            ...
            <a-tab-pane key="4" title="Lolisuki">
              ...
              <a-form-item field="lolisuki.commands" label="指令" tooltip="lolisuki涩图指令" extra="输入一个指令后按下Enter添加" :disabled="!formModel.lolisuki?.enable" feedback>
                <a-input-tag v-model:model-value="formModel.lolisuki!.commands" :style="{ minHeight: '100px' }" placeholder="输入指令后按下回车添加" allow-clear />
              </a-form-item>
              <a-form-item label="级别范围" tooltip="涩图级别，值越大表示越涩，最小为0，最大为6，0~4为非R18，5~6为R18" :disabled="!formModel.lolisuki?.enable" feedback>
            ...
            <a-tab-pane key="5" title="本地涩图">
              ...
              <a-form-item field="local.commands" label="指令" tooltip="本地涩图指令" extra="输入一个指令后按下Enter添加" :disabled="!formModel.local?.enable" feedback>
                <a-input-tag v-model:model-value="formModel.local!.commands" :style="{ minHeight: '100px' }" placeholder="输入指令后按下回车添加" allow-clear />
              </a-form-item>
              <a-form-item field="local.localPath" label="图片目录" tooltip="本地涩图的存放目录" :disabled="!formModel.local?.enable" feedback>
            ...
            <a-tab-pane key="6" title="画师作品预览">
              ...
              <a-form-item field="pixivUser.previewInPage" label="每页数量" tooltip="合成一览图中，每张一览图最多显示多少个作品" :disabled="!formModel.pixivUser?.enable" feedback>
                <a-input-number v-model:model-value="formModel.pixivUser!.previewInPage" :style="{ maxWidth: '300px' }" :min="0" :max="100" placeholder="输入一个数字" mode="button" size="large">
                </a-input-number>
              </a-form-item>
