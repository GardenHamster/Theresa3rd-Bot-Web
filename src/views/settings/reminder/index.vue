    ...
    <div class="container">
      <a-form ref="formRef" layout="horizontal" size="large" :auto-label-width="true" :scroll-to-first-error="true" :model="formModel">
        ...
        <a-card class="card" v-for="(item, timerIndex) of  formModel.timers " :key="timerIndex" :title="`模板${padLeft((timerIndex + 1).toString(), (0).toString(), 2)}`">
          ...
          <a-form-item :field="`timers[${timerIndex}].enable`" label="开启" tooltip="是否启用该定时器" :disabled="!formModel.enable" feedback>
          ...
          <a-form-item :field="`timers[${timerIndex}].name`" label="名称" tooltip="定时器名称" :disabled="!formModel.enable || !item.enable" :rules="[{ required: true, message: '必须输入名称' }]" feedback>
            <a-input v-model:model-value="item.name" placeholder="输入一个名称" allow-clear />
          </a-form-item>

          <a-form-item :field="`timers[${timerIndex}].cron`" label="Cron" tooltip="定时器Cron表达式，详细可以百度Cron在线生成" :disabled="!formModel.enable || !item.enable"
        ...
        <a-card class="card addCard" size="small" :body-style="{ padding: '0px', height: '100%' }" :style="{ marginTop: '5px', marginBottom: '20px' }" @click="onCreateCard">
