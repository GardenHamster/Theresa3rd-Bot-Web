...
import { getSaucenaoCookie, setSaucenaoCookie, CookieData } from '@/api/cookie';
...
const onSubmit = async () => {
  try {
    ...
    setLoading(true);
    await setSaucenaoCookie(formModel.value.cookie);
...
const fetchCookie = async () => {
  try {
    setLoading(true);
    formModel.value = await getSaucenaoCookie() as unknown as CookieData;
    initModel.value = JSON.parse(JSON.stringify(formModel.value))
  } catch (error) {
    console.log(error);
  }
