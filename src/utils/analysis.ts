...
const createImage = async (path: string): Promise<PreviewContent> => {
...
const splitImgCode = async (str: string): Promise<PreviewContent[]> => {
    let textString = str;
    ...
    for (let i = 0; i < array.length; i += 1) {
        ...
        if (imgStr.length > 0) contents.push(await createImage(imgStr));
...
const splitImgCodes = async (contents: PreviewContent[]): Promise<PreviewContent[]> => {
    ...
    for (let index = 0; index < contents.length; index += 1) {
        ...
        if (content.type === PreviewType.Text) {
            ...
            const splitContents = await splitImgCode(textValue.text);
...
export const analysis = async (template: string): Promise<PreviewContent[]> => {
    let contents: PreviewContent[] = [];
    contents = splitNewLine(template);
    console.log('contents', contents);
    contents = await splitImgCodes(contents);
    console.log('contents', contents);
    return contents;
}
